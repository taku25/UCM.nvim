...
function M.resolve_locations(target_dir)
  ...
  if not target_dir then return default_list, default_list end
      ...
      if component:match(rule.regex) then
        ...
        if vim.fn.isdirectory(alternate_path) == 1 then
            ...
            elseif rule.type == "source" then
                ...
                -- 指定なし
                if not seen_header[norm_alt] then table.insert(header_candidates, norm_alt) end
                if not seen_source[norm_alt] then table.insert(source_candidates, norm_alt) end
            end
        end
      end
    end
  ...
  if #header_candidates == 0 then table.insert(header_candidates, normalized_target) end
  if #source_candidates == 0 then table.insert(source_candidates, normalized_target) end
