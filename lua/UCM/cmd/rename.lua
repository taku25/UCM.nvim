...
function M.run(opts, on_complete)
  ...
  if old_class_name == new_class_name then
    return on_complete(false, "canceled") -- Cancel quietly if names are the same
  end
  ...
  if #files_to_process == 0 then
    return on_complete(false, "No existing files found to rename.")
  end
  ...
  for _, old_path in ipairs(files_to_process) do
    ...
    local new_path = fs.joinpath(dir, new_class_name .. "." .. ext)
  ...
  local prompt_str = string.format("Rename '%s' to '%s'?\n\n%s", old_class_name, new_class_name, detailed_changes_str)
  ...
  vim.ui.select(
    ...
    function(choice)
      ...
      for _, path in ipairs(files_to_process) do
        ...
        if not ok then
          return on_complete(false, "Content replacement failed: " .. replace_err)
        end
      end
      ...
      for _, old_path in ipairs(files_to_process) do
        ...
        if rename_ok then
          ...
          return on_complete(false, "File rename operation failed: " .. tostring(rename_err))
        end
      end

      logger.info("Rename complete. IMPORTANT: Please regenerate your project files.")
      on_complete(true, class_info) -- Success!
    end
  )
end
